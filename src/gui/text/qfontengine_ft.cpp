...
QFontGlyph* QFontEngineFT::getGlyph(glyph_t glyph) const
{
	...
	gcache->outline.contours = static_cast<short unsigned int*>(::malloc(sizeof(short) * n_contours));
    ...
    ::memcpy(gcache->outline.points, face->glyph->outline.points, sizeof(FT_Vector) * n_points);
        gcache->outline.tags = static_cast<unsigned char*>(::malloc(sizeof(char) * n_points));
